...
def call_zhipu_api(messages, model="glm-4-flash"):
    ...
    data = {
        "model": model,
        "messages": messages,
        "temperature": 0.7  # 提高一点温度，让风格更灵活自然
    }
...
def roles(role_name):
    ...
    # 如果有外部记忆，优先使用记忆内容
    if memory_content:
            role_prompt_parts.append(f"""【你的说话风格示例 - 最高优先级】
            以下是你说过的真实对话记录，这些是你的说话风格和语气的标准参考：
            
            {memory_content}
            
            【严格要求】
            1. 你必须严格模仿上述示例中的说话风格、语气、用词习惯
            2. 注意示例中的特点：
               - 语气词的使用（如"喔哦哦哦"、"哈哈哈哈"）
               - 口语化表达（如"可以可以"、"我真服了"）
               - 句子长度和结构
               - 标点符号的使用习惯
            3. 在每次回复中，都要体现出与上述示例一致的说话风格
            4. 这是最高优先级要求，比角色设定更重要""")
